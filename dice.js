/*
 */

var dice
var r = 1
let canvHeight = () => calculateCanvHeight(r)
let canvWidth = 225

function calculateCanvHeight(n) {
    if (n < 4) {
        return 80
    }
    else {
        return 160
    }
}

const pips = [
    { id: 0, coords: [1, 1], faces: [2, 3, 4, 5, 6]},
    { id: 1, coords: [1, 3], faces: [6]},
    { id: 2, coords: [1, 5], faces: [4, 5, 6]},
    { id: 3, coords: [3, 5], faces: []},
    { id: 4, coords: [5, 5], faces: [2, 3, 4, 5, 6]},
    { id: 5, coords: [5, 3], faces: [6]},
    { id: 6, coords: [5, 1], faces: [4, 5, 6]},
    { id: 7, coords: [3, 1], faces: []},
    { id: 8, coords: [3, 3], faces: [1, 3, 5]}
]

const drawSquare = (x,y,w,color) => {
    let canv = document.getElementById("diceGameCanvas")
    let ctx = canv.getContext("2d")
    ctx.fillStyle=color
    ctx.fillRect(x,y,w,w)
}

const drawFace = (offset, roll) => {
        for (var j = 0; j < pips.length; j++) {
            if ( pips[j].faces.includes(roll) ) {
                //console.log(pips[j].coords)
                if (offset > 3) {
                    let a = ((offset - 4)*75 + pips[j].coords[0]*10)
                    let b = (offset + 80 + pips[j].coords[1]*10)
                    let c = 10
                    drawSquare(a,b,c,'lime')
                    //console.log(a,b,c)
                }
                else {
                    let a = ((offset - 1)*75 + pips[j].coords[0]*10)
                    let b = (offset + pips[j].coords[1]*10)
                    let c = 10
                    drawSquare(a,b,c,'lime')
                    //console.log(a,b,c)
                }
            }
        }
}

const rollDie = () => Number(Math.floor(Math.random() * 6) + 1)

const rollDice = (dice) => {
    let total = 0
    let printStr = ''
    let diceTotalsView = document.getElementById("diceGameTotals")
    resetCanvas()
    for (i = 1; i <= dice; i++) {
        var roll = rollDie()
        total += roll
        printStr = printStr + roll
        drawFace(i,roll)
        if (i < dice) {
        printStr = printStr + ' + '
        }
    }
    if (dice > 1) {
        //printStr = printStr + ' = ' + total 
        diceTotalsView.textContent = 'Total: ' + total
        return printStr 
    }

    diceTotalsView.textContent = '   Total:  ' + total
    return total
}

//let resetCanvas =  () => {
function resetCanvas () {
    let canv=document.getElementById("diceGameCanvas")
    let ctx=canv.getContext("2d")
    canv.height = calculateCanvHeight(r)
    canv.width = canvWidth
    ctx.height = calculateCanvHeight(r)
    ctx.width = canvWidth
    ctx.fillStyle="black"
    //console.log("height",ctx.height)
    //ctx.fillRect(0,0,canv.width,canv.height)
    ctx.fillRect(0,0,canv.width,canv.height)
}

//let resetBoard = () => {
function resetBoard () {
    createAllElements()
    resetCanvas()
    drawButtons()
    rollDice(r) 
}

function createAllElements () {
    let b = document.getElementById("diceBoard")
    b.className += 'container'

    const d = document.createElement("div")
    d.id = ("diceGameControlsWrapper")
    //controlBoard.className += 'col-md-4 m-md-auto'
    d.className += 'row'
    b.appendChild(d)

    const ctrls = document.createElement("div")
    ctrls.id = ("diceGameController")
    //ctrls.className += 'col-md-auto'
    ctrls.className += 'col-sm'
    //ctrls.className += 'col-md-6 float-left'
    d.appendChild(ctrls)

    const totals = document.createElement("div")
    totals.id = ("diceGameTotals")
    //totals.className += 'col-md-auto'
    totals.className += 'col-sm'
    //totals.className += 'text-right'
    d.appendChild(totals)


    let c = document.createElement("canvas")
    c.id = ("diceGameCanvas")
    c.height = calculateCanvHeight(r)
    c.width = canvWidth
    b.appendChild(c)
}

const drawButtons = () => {
    var controlBoard = document.getElementById("diceGameController")

    var btn = document.createElement("button")
    btn.innerHTML = ('+')
    btn.className += 'btn btn-sm btn-outline-success'
    btn.addEventListener("click", () => {
        if (r < 6) {
            r++
            reDrawRollBtn()
            rollDice(r)
        }
    })
    controlBoard.appendChild(btn)

    var btn2 = document.createElement("button")
    btn2.innerHTML = ('-')
    btn2.className += 'btn btn-sm btn-outline-success'
    btn2.addEventListener("click", () => {
        if (r > 1) {
            r--
            reDrawRollBtn()
            rollDice(r)
        }
    })
    controlBoard.appendChild(btn2)

    var rollBtn = document.createElement("button")
    rollBtn.innerHTML = ('roll ' + r)
    rollBtn.id = ("myRollButton")
    rollBtn.className += 'btn btn-sm btn-success'
    rollBtn.addEventListener("click", () => { rollDice(r); })
    controlBoard.appendChild(rollBtn)

    function reDrawRollBtn() {
        document.getElementById("myRollButton").innerHTML = ('roll ' + r)
    }

}

window.onload = () => resetBoard()

